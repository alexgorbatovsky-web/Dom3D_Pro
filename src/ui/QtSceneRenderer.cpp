...
void QtSceneRenderer::Render(const CAlfaDoc& document,
    ...
    if (orthographic) {
        ...
        Perspective(camera.vertical_fov_degrees, aspect, z_near, z_far);
    }
...
void QtSceneRenderer::CalculateClipPlanes(const CAlfaDoc& document,
    ...
    // Architectural models use millimeters.  A sub-millimeter near plane
    // combined with a room-sized far plane wastes almost the entire 24-bit
    // depth buffer and makes coplanar cabinet parts flicker through each
    // other.  Tie the perspective near plane to the view scale so the same
    // precision survives when an interior Walk camera switches back to Orbit.
