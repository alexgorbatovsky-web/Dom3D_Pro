...
public:
	...
	void SwapValues() {
		cMath::Swap(m_StartValue, m_EndValue);
	}
private:
	float m_StartTime;
	float m_Duration;
...
// cInterpolate<Type>::Clear
template<class Type>
inline void cInterpolate<Type>::Clear() {
	m_StartTime = m_Duration = m_CurrentTime = 0.0f;
	memset(&m_StartValue, 0, sizeof(m_StartValue));
...
public:
	...
	bool IsDone(const float Time) const {
		return Time >= m_StartTime + m_AccelTime + m_LinearTime + m_DecelTime;
	}

	void SetStartTime(const float StartTime) {
		m_StartTime = StartTime; Invalidate();
	}
	void SetAccelTime(const float AccelTime) {
		m_AccelTime = AccelTime;
	}
	void SetDecelTime(const float DecelTime) {
		m_DecelTime = DecelTime;
	}
	void SetStartValue(const Type &StartValue) {
		m_StartValue = StartValue; Invalidate();
	}
	void SetEndValue(const Type &EndValue) {
		m_EndValue = EndValue; Invalidate();
	}
	...
	float GetEndTime() const {
		return m_StartTime + m_AccelTime + m_LinearTime + m_DecelTime;
	}
	float GetDuration() const {
		return m_AccelTime + m_LinearTime + m_DecelTime;
	}
...
// cInterpolateAccelDecelLinear<Type>::Clear
template<class Type>
inline void cInterpolateAccelDecelLinear<Type>::Clear() {
...
// cInterpolateAccelDeclSine<Type>::Clear
template<class Type>
inline void cInterpolateAccelDecelSine<Type>::Clear() {
	m_StartTime = m_AccelTime = m_LinearTime = m_DecelTime = 0.0f;
	memset(&m_StartValue, 0, sizeof(m_StartValue));
	m_EndValue = m_StartValue;
	m_Extrapolate.Clear();
